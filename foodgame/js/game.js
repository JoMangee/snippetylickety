...
async function loadAll() { const key = savedKey() || apiKeyValue(); if (!key) throw new Error('Add the API key in Settings first.'); const [stats, feed, activity, effects] = await Promise.all([get(key, { action: 'stats', player: currentPlayer() }), get(key, { action: 'feed' }), get(key, { action: 'activity' }), get(key, { action: 'effects', player: currentPlayer() })]); renderStats(stats); renderHistory(activity.entries || []); renderFeed(feed.entries || []); renderActivity(activity.entries || []); renderBuff(effects); lastActivityId = activity.next_since || lastActivityId; setConnected(true); setStatus('Stats, history, and feed loaded.', true); return effects; }
