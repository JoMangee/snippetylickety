...
async function logMeal(event) { event.preventDefault(); setBusy(true); try { const key = savedKey() || apiKeyValue(); if (!key) throw new Error('Add the API key in Settings first.'); storePlayer(); const result = await get(key, { action: 'log', ...formValues(), player: playerValue() }); const effects = await loadAll(); renderBuff(effects); setConnected(true); const level = result.stats?.level; setStatus(level === undefined ? 'Logged!' : 'Logged! +' + result.xp_gained + ' XP level ' + level + '.', true); } catch (error) { setConnected(false); setStatus(error.message); } finally { setBusy(false); } }
