...
The root `cpanel.yml` deployment task copies `foodgame/*` to `/home2/meshnet/bbb.mesh.net.nz/foodgame/` and applies the required directory and file permissions. HTTPS remains mandatory.
...
Every API request requires the `key` query parameter. A missing or incorrect key returns HTTP 401 and {"ok":false,"opt":"unauthorized"}`. The API uses `hash_equals` for constant-time comparison.
...
Use an HTTPS-serving local reverse proxy or another HTTP Test server; donot expose the game through plain HTTP. With a private config in place, verify the API with the HTTPS URL for that server, for example:
...
The rate limiter is file based: authorized requests receive a per-IP rolling limit of 60 requests per minute. Game data written under an exclusive lock and saved by temporary-file-plus-rename atomic replacement. This is intentionally small-device storage, not a multi-server database.
