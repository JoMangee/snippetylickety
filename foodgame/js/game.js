...
import { apiKeyValue, formValues, on, playerValue as uiPlayerValue, populateMeals, renderActivity, renderBuff, renderFeed, renderHistory, renderStats, renderActiveBadge, setApiKeyValue, setBusy, setConnected, setPlayerValue, setStatus, toggleAddMeal } from './ui.js';
...
let chargeState={active:false,timer:null,retryTimer:null,remaining:0};
function renderCharge(m='Ready-to-log'){let x=document.getElementById('charge-status'),b=document.querySelector('#meal-form>button[type="submit"]');if(x){x.textContent=m;x.classList.toggle('charge-active',chargeState.active)}if(b)b.disabled=chargeState.active;}
function stopChargeMode(){if(chargeState.timer)clearTimeout(chargeState.timer);if(chargeState.retryTimer)clearTimeout(chargeState.retryTimer);chargeState.active=false;chargeState.timer=chargeState.retryTimer=null;chargeState.remaining=0;renderCharge();}
function startCharge(d=850){chargeState.active=true;chargeState.remaining=Math.ceil(d/1000);renderCharge('Charging...');}
function scheduleCooldown(v){let r=Math.max(0,Math.ceil(Number(v)||0));chargeState.remaining=r;chargeState.active=r>0;if(chargeState.retryTimer)clearInterval(chargeState.retryTimer);if(!r){stopChargeMode();return;}renderCharge('Cooldown:'+r+'s');chargeState.retryTimer=setInterval(()=>{chargeState.remaining-=1;if(chargeState.remaining<=0){stopChargeMode();syncCooldown();}else renderCharge('Cooldown:'+chargeState.remaining+'s');},1000);}
async function syncCooldown(){let k=savedKey()||apiKeyValue();if(!k){stopChargeMode();return;}try{let r=await get(k,{action:'cooldown',player:currentPlayer(),meal:formValues().meal});let v=r.cooldown||r.cooldown_seconds||r.remaining_seconds||0;if(v)scheduleCooldown(v);else stopChargeMode();}catch(e){if(e.status!==429)stopChargeMode();}}
function flashSuccess(m='Meal-logged!'){let x=document.getElementById('charge-status');if(x){x.textContent=m;x.classList.add('success-flash');setTimeout(()=>x.classList.remove('success-flash'),900);}}
async function loadBuildVersion(){try{let r=await fetch('index.meta.json',{cache:'no-store'});let m=await r.json();let x=document.getElementById('build-version');if(x)x.textContent='Build'+(m.version||'unknown');}catch(e){let x=document.getElementById('build-version');if(x)x.textContent='Build-unavailable';}}
async function loadAll() { const key = savedKey() || apiKeyValue(); if (!key) throw new Error('Add the API key in Settings first.'); const [stats, feed, activity, effects] = await Promise.all([get(key, { action: 'stats', player: currentPlayer() }), get(key, { action: 'feed' }), get(key, { action: 'activity' }), get(key, { action: 'effects', player: currentPlayer() })]); renderStats(stats); renderHistory(activity.entries || []); renderFeed(feed.entries || []); renderActivity(activity.entries || []); renderBuff(effects); const lastEntry = activity.entries?.at(-1); const fallback = lastEntry?.summary?.meal_name || lastEntry?.meal || 'NOODLE MASTERPIECE'; renderActiveBadge(effects?.effects?.[0]?.keyword || fallback); lastActivityId = activity.next_since || lastActivityId; setConnected(true); setStatus('Stats, history, and feed loaded.', true); await syncCooldown(); return effects; }
...
async function logMeal(event) { event.preventDefault(); if (chargeState.active) return; startCharge(850); setBusy(true); try { const key = savedKey() || apiKeyValue(); if (!key) throw new Error('Add the API key in Settings first.'); storePlayer(); await new Promise(resolve => setTimeout(resolve, 850)); chargeState.remaining = 0; const result = await get(key, { action: 'log', ...formValues(), player: playerValue() }); const effects = await loadAll(); renderBuff(effects); setConnected(true); const level = result.stats?.level; setStatus(level === undefined ? 'Logged!' : 'Logged! +' + result.xp_gained + ' XP level ' + level + '.', true); flashSuccess(); } catch (error) { if (error && error.status === 429) { const key = savedKey() || apiKeyValue(); try { const c = await get(key, { action: 'cooldown', player: currentPlayer(), meal: formValues().meal }); scheduleCooldown(c.cooldown || 30); } catch (e) { scheduleCooldown(30); } setStatus('On cooldown. Try again shortly.', true); } else { setConnected(false); setStatus(error.message); } } finally { setBusy(false); if (!chargeState.remaining) stopChargeMode(); } }
